...
= 1.1.39 =
* Fix expired-link error when deleting a lead from the CRM card (nonce URL was HTML-escaped in JSON).
* Same nonce URL builder for form duplicate/export/delete, test email, and CSV export.

= 1.1.38 =
* CRM lead card: show Save/Cancel only in edit mode (Edit hides while editing).
...
* Design prompt includes intro, all field details, button text, success message, and styles.
* Code checker validates intro, submit control, success block, steps, field names, and option texts.
* Legacy constructor “button” fields are migrated into the submit label setting.
